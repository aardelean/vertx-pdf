...
class BlockingLocalTemplatePdfHandler implements Handler<RoutingContext> {
    ITextRenderer renderer = new ITextRenderer();
    @Override
    void handle(RoutingContext ctx) {
        ByteArrayOutputStream baos = new ByteArrayOutputStream()
        ctx.vertx().executeBlocking({future ->
            String body = BlockingLocalTemplatePdfHandler.class.classLoader.getResourceAsStream("invoice.html").getText("utf-8")
            renderPdf(body, baos)
            future.complete();
        }, {res ->
            if (res.succeeded()) {
